...
func newDmIntervalDTByBytes(bytes []byte) *DmIntervalDT {
	...
	dt._type = bytes[21]
	dt.scaleForSvr = int(Packet.Dm_build_103(bytes, 20))
	dt.leadScale = (dt.scaleForSvr >> 4) & 0x0000000F
	dt.secScale = dt.scaleForSvr & 0x0000000F

	switch dt._type {
	case QUA_D:
		dt.days = int(Packet.Dm_build_103(bytes, 0))
	case QUA_DH:
		dt.days = int(Packet.Dm_build_103(bytes, 0))
		dt.hours = int(Packet.Dm_build_103(bytes, 4))
	case QUA_DHM:
		dt.days = int(Packet.Dm_build_103(bytes, 0))
		dt.hours = int(Packet.Dm_build_103(bytes, 4))
		dt.minutes = int(Packet.Dm_build_103(bytes, 8))
	case QUA_DHMS:
		dt.days = int(Packet.Dm_build_103(bytes, 0))
		dt.hours = int(Packet.Dm_build_103(bytes, 4))
		dt.minutes = int(Packet.Dm_build_103(bytes, 8))
		dt.seconds = int(Packet.Dm_build_103(bytes, 12))
		dt.fraction = int(Packet.Dm_build_103(bytes, 16))
	case QUA_H:
		dt.hours = int(Packet.Dm_build_103(bytes, 4))
	case QUA_HM:
		dt.hours = int(Packet.Dm_build_103(bytes, 4))
...
func (dt *DmIntervalDT) encode(scale int) ([]byte, error) {
	...
	bytes := make([]byte, 24)
	if dt.negative {
		Packet.BuildPacket_Dm_build_17(bytes, 0, int32(-day))
		Packet.BuildPacket_Dm_build_17(bytes, 4, int32(-hour))
		Packet.BuildPacket_Dm_build_17(bytes, 8, int32(-minute))
		Packet.BuildPacket_Dm_build_17(bytes, 12, int32(-second))
		Packet.BuildPacket_Dm_build_17(bytes, 16, int32(-f))
		Packet.BuildPacket_Dm_build_17(bytes, 20, int32(scale))
	} else {
		Packet.BuildPacket_Dm_build_17(bytes, 0, int32(day))
		Packet.BuildPacket_Dm_build_17(bytes, 4, int32(hour))
		Packet.BuildPacket_Dm_build_17(bytes, 8, int32(minute))
