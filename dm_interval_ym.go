...
func newDmIntervalYMByBytes(bytes []byte) *DmIntervalYM {
	...
	ym.scaleForSvr = int(Packet.Dm_build_103(bytes, 8))
	ym.leadScale = (ym.scaleForSvr >> 4) & 0x0000000F
	ym._type = bytes[9]
	switch ym._type {
	case QUA_Y:
		ym.years = int(Packet.Dm_build_103(bytes, 0))
	case QUA_YM:
		ym.years = int(Packet.Dm_build_103(bytes, 0))
		ym.months = int(Packet.Dm_build_103(bytes, 4))
	case QUA_MO:
		ym.months = int(Packet.Dm_build_103(bytes, 4))
	}
	return ym
...
func (ym *DmIntervalYM) encode(scale int) ([]byte, error) {
	...
	bytes := make([]byte, 12)
	Packet.BuildPacket_Dm_build_17(bytes, 0, int32(year))
	Packet.BuildPacket_Dm_build_17(bytes, 4, int32(month))
	Packet.BuildPacket_Dm_build_17(bytes, 8, int32(scale))
	return bytes, nil
}
