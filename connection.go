...
func (conn *DmConnection) setTrxFinish(status int32) {
	switch status & Status0x00000FFF {
	case Status0x00000000, Status0x00000020, Status0x00000040:
		conn.trxFinish = true
	default:
...
func (dc *DmConnection) executeInner(query string, execType int16) (interface{}, error) {
	...
	if execType == ExecType01 && dc.dmConnector.enRsCache {
...
func (dc *DmConnection) exec(query string, args []driver.Value) (*DmResult, error) {
	...
	if args != nil && len(args) > 0 {
		...
		r1, err := dc.executeInner(query, ExecType02)
		if err != nil {
			return nil, err
		}
...
func (dc *DmConnection) query(query string, args []driver.Value) (*DmRows, error) {
	...
	if args != nil && len(args) > 0 {
		...
		r1, err := dc.executeInner(query, ExecType01)
		if err != nil {
			return nil, err
		}
...
func (dc *DmConnection) driverQuery(query string) (*DmStatement, *DmRows, error) {
	...
	stmt.innerUsed = true
	stmt.innerExec = true
	info, err := dc.Access.Dm_build_782(stmt, ExecType01)
	if err != nil {
		return nil, nil, err
	}
	dc.lastExecInfo = info
